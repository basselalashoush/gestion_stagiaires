...
const modifier = function(event) {
    let element = event.target;
    var id_stagiaire = element.getAttribute('data-id-stagiaire');
    var id_formation = document.querySelector(`[data-stagiaire="${id_stagiaire}"]`).value;
    let periodesStagiaire = document.querySelectorAll(`[data-id="${id_stagiaire}"]`);
    periodesStagiaire.forEach(function(periode) {
        if (periode.checked) {
            let id_periodes = periode.getAttribute('data-periode');
        }
    })
    sendData(table);
}
...
setInterval(function() {
    ...
    for (var i = 0; i < buttons.length; i++) {
        buttons[i].addEventListener('click', modifier, false);
    }
